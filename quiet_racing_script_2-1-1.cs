...
private float[][] frontWheelSpeeds = {
    new float[] {25f, 70f, 95f, 100f},
    new float[] {25f, 70f, 95f, 100f}
};
// Dependent Variable (y)
private float[][] frontWheelAngles = {
    new float[] {40f, 46f, 35.3f, 32f},
    new float[] {40f, 46f, 35.3f, 32f}
};
private float[][] rearWheelSpeeds = {
    new float[] {25f, 70f, 70f},
    new float[] {25f, 70f, 70f}
};
...
private float neutralHeightReturnDelay = 1f; // Time in seconds before the height returns to the neutral height
...
// Active Strength
private float strengthDelta = 1f;
private float neutralStrengthReturnDelay = 1f; // Time in seconds before the strength returns to the current neutral strength
private float[][] strengthSpeeds = {
    new float[] {0f, 50f, 100f}
};
private float[][] neutralStrengths = {
    new float[] {30f, 16f, 10f}
};
private float[][] frontInsideTurningStrengths = {
	new float[] {30f, 16f, 10f}
};
private float[][] frontOutsideTurningStrengths = {
	new float[] {20f, 12f, 8f}
};
...
private float[][] ERSSpeeds = {
    new float[] {0f, 97.5f},
    new float[] {0f, 88f, 92f, 97.5f},
    new float[] {0f, 88f, 92f, 97.5f},
    new float[] {0f, 85f, 93f, 97.5f}
};
...
private float powerPercentageCheck = 0.48f; // 0.75f is 75% power usage
...
private string QRSVersion = "2.1.1";
...
private void HandleArgument(string argument) {
    ...
    for (int i = 0; i < splitArgument.Length; i++) {
        ...
        if (TryTireArgument(s) && !suspensionPitModeSet) { continue; }

        if (s == "FLIP") { isDoingFlip = true; continue; }
    }
}

private bool TryTireArgument(string compound) {
...
private void HandleActiveStrength() {
    ...
    if (Math.Abs(xDirection) > 0) {
        ...
        frStr = GoToValue(_suspensions[0].Strength, frStr, strengthDelta);
        rlStr = GoToValue(_suspensions[0].Strength, rlStr, strengthDelta);
        rrStr = GoToValue(_suspensions[0].Strength, rrStr, strengthDelta);
    }
